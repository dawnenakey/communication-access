...
export default function ConversationMode() {
  ...
  const loadAvailableSigns = async () => {
    try {
      ...
      if (data?.signs) {
        ...
        setSignCount(data.count || data.signs.length);
      }
    } catch (error) {
      // Fallback to legacy endpoint
      try {
        const fallbackResponse = await fetch(`${API}/conversation/signs/available`);
        const fallbackData = await fallbackResponse.json();
        setAvailableSigns(fallbackData.signs || []);
        setSignCount(fallbackData.signs?.length || 50);
      } catch (fallbackError) {
  ...
  const stopRecording = useCallback((frames: string[] | null = null) => {
  ...
  const processConversation = async (frames: string[] | null, text: string | null) => {
